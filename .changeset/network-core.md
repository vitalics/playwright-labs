---
"@playwright-labs/network-core": major
---

Initial release — network condition emulation for Chromium over CDP.
...
- `NetworkAPI` — `start(preset | condition)` applies and returns the resolved condition, `stop()` restores normal networking (idempotent), `condition`/`started` getters, `await using` support (async dispose restores networking on scope exit); the CDP `Network` domain is enabled lazily once
...
await network.start(createNetworkCondition({        // custom
  latency: 800,
...
**NOTE:* Chromium only — CDP sessions are not available for Firefox/WebKit.
