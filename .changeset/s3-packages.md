...
"@playwright-labs/reporter-s3": minor
"@playwright-labs/fixture-s3": minor
---

Initial release of the S3 package family — ship Playwright run artifacts to any S3-compatible storage (AWS S3, MinIO, Cloudflare R2, ...).
...
await client.putObject("artifacts", "runs/report.json", JSON.stringify(data), {
...
test("uploads run artifacts", async ({ useBucket }) => {
  ...
  const log = bucket.createWriteStream({ name: "run.log", contentType: "text/plain" });
  log.write("step 1\n");
  log.end("step 2\n");
  await log.done;
});
