...
```
live DB  ──►  pnpm sql-core pull  ──►  db-types.ts  ──►  tsconfig "schemaFile"  ──►  editor features
                                            │
...
Use the `pull` CLI from `@playwright-labs/sql-core` to introspect a live database and generate typed row interfaces:
...
pnpm sql-core pull --adapter mysql  --url mysql://user:pass@localhost/mydb --out ./src/db-types.ts
```

The CLI requires `@playwright-labs/sql-core` installed as a direct devDependency — pnpm only links bins of direct dependencies into `node_modules/.bin`:
