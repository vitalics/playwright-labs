...
export default class PrometheusReporter extends BaseReporter {
  ...
  constructor(options: PrometheusOptions = {} as PrometheusOptions) {
    ...
    this.options.url =
      ...
      (() => {
        throw new TypeError(
          `'serverUrl' is not defined for '@playwright-labs/reporter-prometheus-remote-write' package. You can set it by following example:
...
`,
  ...
  /**
   * Pushes timeseries to the Prometheus remote write endpoint.
   * Override this in test subclasses to capture series instead of pushing to Prometheus.
   *
   * Series without samples are skipped: metrics are drained (only unsent
   * samples are included), so a drained metric with no new samples would
   * produce an empty series that Prometheus rejects.
   */
  protected async send(series: Timeseries | Timeseries[]) {
    const batch = (Array.isArray(series) ? series : [series]).filter(
      (s) => s.samples.length > 0,
    );
    if (batch.length === 0) {
      return;
    }
    await pushTimeseries(batch, this.options);
  }
  ...
  async onTestEnd(test: TestCase, result: TestResult) {
    ...
    const annotationSeries: Timeseries[] = [];
  ...
  private mapTimeseries(
    ...
    if (seriesOrMetric instanceof Counter || seriesOrMetric instanceof Gauge) {
      // Drain: only samples not pushed before are included. Prometheus rejects
      // the whole remote-write request when it carries already-pushed samples
      // ("out of order sample"), so the full sample history must not be resent.
      series = seriesOrMetric._drainSeries();
    }
