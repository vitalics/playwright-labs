...
export interface PlaywrightReportShadcnButtonEmailProps {
  result: FullResult;
  testCases: NodemailerTestCases;
  /** URL to the full HTML report (e.g. your CI artifact link). */
  reportUrl?: string;
  /** URL pointing directly to the failures page / filtered view. */
  failuresUrl?: string;
}

export function PlaywrightReportShadcnButtonEmail({
  result,
  testCases,
  reportUrl,
  failuresUrl,
}: PlaywrightReportShadcnButtonEmailProps): React.ReactElement {
  ...
  const hasButtons = reportUrl || (failuresUrl && failed > 0);
        ...
        e("div", { style: { maxWidth: "600px", margin: "32px auto", padding: "0 16px" } },
          e(Card, { className: "overflow-hidden" },
            ...
            hasButtons && e(CardFooter, {
              ...
              reportUrl && e(Button, { href: reportUrl, variant: "default", size: "sm" }, "View Full Report"),
              failuresUrl && failed > 0 && e(Button, { href: failuresUrl, variant: "destructive", size: "sm" }, `View ${failed} Failure${failed !== 1 ? "s" : ""}`),
            ),
          ),
        ),
      ),
...
export default PlaywrightReportShadcnButtonEmail;
