...
- 3d0b324: Initial release — network throttling fixtures for Playwright with custom matchers, built on `@playwright-labs/network-core` (CDP, Chromium only).
  ...
  **Fixtures**
  ...
  test("throttle a second page", async ({ context, useNetwork }) => {
    ...
    await network.start(
      createNetworkCondition({ latency: 800, downloadThroughput: kbps(256) }),
    );
  });
  ```
...
- Updated dependencies [3d0b324]
  - @playwright-labs/network-core@1.0.0
